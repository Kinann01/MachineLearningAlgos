...
def main(args: argparse.Namespace) -> np.ndarray:
    ...
    while not done:
        ...
        # Shuffle the data and the target based on the permutation function
        data_shuffled = data[permutation]
        target_shuffled = target[permutation]

        for example in range(0, len(data)):
            predictExample = np.dot(data_shuffled[example].T, weights)

            if (predictExample * target_shuffled[example] <= 0):
                weights += target_shuffled[example] * data_shuffled[example]

        done = np.all(np.dot(data_shuffled, weights) * target_shuffled > 0)
