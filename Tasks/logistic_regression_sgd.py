...
def main(args: argparse.Namespace) -> tuple[np.ndarray, list[tuple[float, float]]]:
    ...
    for epoch in range(args.epochs):
        ...
        def NumCorrectPredictions(predict_ , test_ ):
            helper = 0
            for i in range(len(test_)):
                if (predict_[i] > 0.5 and test_[i] == 1):
                    helper += 1
                if (predict_[i] <= 0.5 and test_[i] == 0):
                    helper += 1
            return helper
            
        predictionWithFinalWeights_train_data = sigmoid(np.dot(train_data, weights))
        almost_accurate_train = NumCorrectPredictions(predictionWithFinalWeights_train_data, train_target)
        train_accuracy =  almost_accurate_train / len(train_target)

        predictionsWithFinalWeights_test_data = sigmoid(np.dot(test_data, weights))
        almost_accurate_test = NumCorrectPredictions(predictionsWithFinalWeights_test_data, test_target)
        test_accuracy = almost_accurate_test / len(test_target)

        # Bernoulli's Distribution & maximum likelihood estimation # Cross Entropy
        def lossFunction(probability, target):
            return np.mean(-target * np.log(probability) - (1 - target) * np.log(1 - probability))

        train_loss = lossFunction(predictionWithFinalWeights_train_data, train_target)
        test_loss = lossFunction(predictionsWithFinalWeights_test_data, test_target)
