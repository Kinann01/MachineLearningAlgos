...
def main(args: argparse.Namespace) -> tuple[tuple[np.ndarray, ...], list[float]]:
    ...
    weightsOfHiddenLayer = weights[0]
    weightsOfOutputLayer = weights[1]
    biasOfHiddenLayer = biases[0]
    biasOfOutputLayer = biases[1]
    ...
    def softmax(z):
        z = np.exp(z - np.max(z, axis=-1, keepdims=True)) # Confirm this function works
    ...
    def forward(inputs):
        ...
        predictionHiddenLayer = np.dot(inputs, weightsOfHiddenLayer) + biasOfHiddenLayer
        hiddenLayerVal = ReLU(predictionHiddenLayer)
        
        predictionOutputLayer = np.dot(hiddenLayerVal, weightsOfOutputLayer) + biasOfOutputLayer
        outputLayerVal = softmax(predictionOutputLayer)
    ...
    for epoch in range(args.epochs):
        ...
        for batch in range(0, len(train_data), args.batch_size):
            ...
            oneDataBatch = data_shuffled[batch : batch + args.batch_size]
            oneTargetBatch = target_shuffled[batch : batch + args.batch_size]

            oneHotTarget = np.eye(args.classes)[oneTargetBatch] # one hot encoding. This is necessary because we are using softmax
            # print(oneHotTarget) 
            
            hiddenLayerGradient = np.zeros_like(weightsOfHiddenLayer)
            OutputLayerGradient = np.zeros_like(weightsOfOutputLayer)
            (hiddenLayerValPred, outputLayerValPred) = forward(oneDataBatch)
    
            # For Hidden Layer
            errorsHidden = np.dot(errorsOutput, weightsOfOutputLayer.T) * dRelu(hiddenLayerValPred)
            hiddenLayerBatchGradient = np.dot(oneDataBatch.T, errorsHidden)
            hiddenLayerGradient = (1 / args.batch_size) * hiddenLayerBatchGradient

            # Hidden layer
            weightsOfHiddenLayer -= args.learning_rate * hiddenLayerGradient
            biasOfHiddenLayer -= args.learning_rate * np.mean(errorsHidden, axis=0)

            # For output Layer
            errorsOutput = outputLayerValPred - oneHotTarget 
            OutputLayerBatchGradient = np.dot(hiddenLayerValPred.T, errorsOutput)
            OutputLayerGradient = (1 / args.batch_size) * OutputLayerBatchGradient

            # Update Weights and bias
            # Output layer
            weightsOfOutputLayer -= args.learning_rate * OutputLayerGradient
            biasOfOutputLayer -= args.learning_rate * np.mean(errorsOutput, axis=0)

        # TODO: After the SGD epoch, measure the accuracy for both the
        # train test and the test set.
        _, outputFinalPredictTrain = forward(train_data)
        _, outputFinalPredictTest = forward(test_data)

        # Predict classes based on maximum probability from the softmax output
        predictedClassesTrain = np.argmax(outputFinalPredictTrain, axis=1)
        predictedClassesTest = np.argmax(outputFinalPredictTest, axis=1)

        # Calculate accuracy as the mean of the correct predictions
        train_accuracy = np.mean(predictedClassesTrain == train_target)
        test_accuracy = np.mean(predictedClassesTest == test_target)
