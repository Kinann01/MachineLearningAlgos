...
def main(args: argparse.Namespace) -> tuple[float, float]:
        ...
        data_target, test_target  = sklearn.model_selection.train_test_split( 
        ...
        # The idea is to go through each hyperparameter lamda and apply it on our weights for weights decay. 
        # Pick a lamda, create our model, get the RMSE, return the best RMSE and the corresponding lambda

        for _lambda in lambdas:
                
                myModel = sklearn.linear_model.Ridge(alpha=_lambda)
                myModel.fit(data_to_train, data_target)

                # Our prediction
                predict = myModel.predict(data_to_test)

                # The RMSE we calculated based on the lambda we chose
                myMse = mean_squared_error(predict, test_target, squared=False)
