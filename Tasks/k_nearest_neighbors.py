...
def getWeights(distance, weightType):

    weights = np.zeros_like(distance)
    if weightType == 'inverse':
        weights = inverse(distance)
    elif weightType == 'softmax':
        weights = softmax(-distance)
    elif weightType =='uniform':
        weights = uniform(distance)
    
    return weights

def getClassCounts(KNearestClasses, neighbours, weights, size):
    classCount = np.zeros(size)
    for index, class_ in enumerate(KNearestClasses):
        neighbour = neighbours[index]
        classCount[class_] += weights[neighbour]
    return classCount
...
def main(args: argparse.Namespace) -> float:
    ...
    for i in range(len(test_data)):
        ...
        distances = distance(train_data, test_data[i], args.p)
        weights = getWeights(distances, args.weights)
        sortedDistances = np.argsort(distances)
        kNearestNeighbours = sortedDistances[:args.k]
        KNearestClasses = train_target[kNearestNeighbours]
        allNumberOfClasses = np.max(train_target) + 1
        classCounts = getClassCounts(KNearestClasses, kNearestNeighbours, weights, allNumberOfClasses)
        theClass = np.argmax(classCounts)
        test_predictions[i] = theClass
