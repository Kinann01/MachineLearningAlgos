...
def main(args: argparse.Namespace) -> list[float]:
    ...
    for order in range(1, args.range + 1):
        ...
        our_feats = np.stack(x, axis=1)

        # TODO: Split the data into a train set and a test set.
        # Use `sklearn.model_selection.train_test_split` method call, passing
        # arguments `test_size=args.test_size, random_state=args.seed`.
        train_data, test_data = sklearn.model_selection.train_test_split(our_feats, test_size=args.test_size, random_state=args.seed)
        ...
        model = lin_reg.fit(train_data, train_target)
        ...
        rmse = sklearn.metrics.mean_squared_error(test_target, predict , squared=False)
        rmses.append(rmse)
