...
def main(args: argparse.Namespace) -> tuple[np.ndarray, list[tuple[float, float]]]:
    ...
    for epoch in range(args.epochs):
        ...
        for i in range(0, train_data.shape[0], args.batch_size):
            ...
            y = softmax(train_data[batch_indices] @ weights)
            g = ((y - np.eye(args.classes)[train_target[batch_indices]]).T @ train_data[batch_indices]).T
            g /= args.batch_size
        ...
        train_loss = -np.mean(np.log(softmax(train_data @ weights))[np.arange(train_data.shape[0]), train_target])
        test_accuracy = np.mean(np.argmax(test_data @ weights, axis=1) == test_target)
        test_loss = -np.mean(np.log(softmax(test_data @ weights))[np.arange(test_data.shape[0]), test_target])
