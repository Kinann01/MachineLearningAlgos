...
def main(args: argparse.Namespace) -> float:
    ...
    train_data, train_target = sklearn.model_selection.train_test_split(dataset.data, test_size=args.test_size, random_state=args.seed)
    test_data, test_target = sklearn.model_selection.train_test_split(dataset.target,test_size=args.test_size, random_state=args.seed)
    ...
    grid_search = sklearn.model_selection.GridSearchCV(estimator=pipeline, param_grid=params_, 
            cv=sklearn.model_selection.StratifiedKFold(n_splits=5), scoring='accuracy')
    
    grid_search.fit(train_data, test_data)
    test_accuracy = grid_search.score(train_target, test_target)
