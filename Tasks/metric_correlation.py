...
def standardDeviation(X, mean):
    return np.sqrt(np.mean(np.square(X - mean)))

def PearsonCorrelation(scores, ratings):
    scoresMean = np.mean(scores)
    ratingsMean = np.mean(ratings)
    scoresAndRatingsMean = np.mean(scores * ratings)
    covariance = scoresAndRatingsMean - (scoresMean * ratingsMean)
    ProductOfStandardDeviation = standardDeviation(scores, scoresMean) * standardDeviation(ratings, ratingsMean)
    return covariance / ProductOfStandardDeviation

def FBetaScore(TP, FP, FN, beta):
    recall = TP / (TP + FN)
    precision = TP / (TP + FP)
...
def main(args: argparse.Namespace) -> tuple[float, float]:
    ...
    for _ in range(args.bootstrap_samples):
        ...
        avergage_human_rating = np.mean([s.human_rating for s in sentences])
        human_ratings.append(avergage_human_rating)
    ...
    for beta in np.linspace(0, 2, 201):
        ...
        scores = [FBetaScore(TP, FP, FN, beta) for (TP, FP, FN) in predictions]
        correlations.append(PearsonCorrelation(np.array(scores), np.array(human_ratings)))
